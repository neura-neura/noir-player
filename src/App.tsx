...
type PanelTab = 'load' | 'playlist' | 'style';

type EndPlaybackAction = 'none' | 'repeat' | 'next';
...
type VideoOpenOptions = {
  panelTab?: PanelTab;
  panelVisible?: boolean;
  playOnReady?: boolean;
  applyOpenPreferences?: boolean;
...
function shouldUseStreamingPlayback(fileName: string): boolean {
  const extension = getFileExtension(fileName);
  return ['ts', 'm2ts'].includes(extension);
}
...
function isEndPlaybackAction(value: string): value is EndPlaybackAction {
  return ['none', 'repeat', 'next'].includes(value);
}

function readStoredEndPlaybackAction(): EndPlaybackAction {
  try {
    const rawValue = window.localStorage.getItem(END_PLAYBACK_ACTION_STORAGE_KEY);
    return rawValue && isEndPlaybackAction(rawValue) ? rawValue : 'none';
  } catch {
    return 'none';
  }
}
...
export default function App() {
  ...
  const videoReadyPanelBehaviorRef =
    useRef<VideoReadyPanelBehavior | null>(null);
  ...
  const activePlaylistIndex = playlistVideos.findIndex((playlistVideo) =>
  ...
  async function openVideoFromPath(
    path: string,
    options?: VideoOpenOptions,
  ) {
  ...
  function closeWebAudioPipeline() {
    ...
    void webAudioRef.current.context.close().catch(() => {
      // Ignore shutdown errors while swapping media elements.
    });
    webAudioRef.current = null;
  }

  function resetMediaElementBeforeSourceSwap() {
    hlsRef.current?.destroy();
    hlsRef.current = null;
    closeWebAudioPipeline();
  ...
  function setVideoInternalAudioMuted(shouldMute: boolean) {
    ...
    for (const audioTrack of browserAudioTracks) {
  ...
  async function playPlaylistVideo(playlistVideo: PlaylistVideo) {
    const preservedPlaybackState = capturePlaybackState();
    setPanelVisible(true);
    ...
    if (areFilePathsEqual(playlistVideo.path, videoSource?.path)) {
      ...
      if (playPromise) {
        await playPromise.catch(() => {
          // The regular controls remain available when the browser blocks play.
        });
      }
      return;
    }

    await openVideoFromPath(playlistVideo.path, {
      panelTab: 'playlist',
      panelVisible: true,
      playOnReady: true,
  ...
  useEffect(() => {
    window.localStorage.setItem(
      END_PLAYBACK_ACTION_STORAGE_KEY,
      endPlaybackAction,
    );
  }, [endPlaybackAction]);
  ...
  useEffect(() => {
    ...
    const shouldPrepareFallback = isCodecLikelyUnsupported(firstTrack.codec);
  ...
  useEffect(() => {
    ...
    async function refreshPlaylist() {
      if (!videoSource?.path || !isDesktopApp()) {
        setPlaylistVideos([]);
      ...
      try {
        const videos = await invoke<PlaylistVideo[]>('list_folder_videos', {
          path: videoSource.path,
        });
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    if (videoSource.kind !== 'hls') {
      if (videoElement.src !== videoSource.src) {
  ...
  useEffect(() => {
    ...
    const logMediaState = (event: Event) => {
      const mediaState = {
        event: event.type,
      ...
      console.debug('[Noir media]', mediaState);
    };
    ...
    events.forEach((eventName) =>
      videoElement.addEventListener(eventName, logMediaState),
    );

    return () => {
      events.forEach((eventName) =>
        videoElement.removeEventListener(eventName, logMediaState),
      );
    };
  }, [videoSource]);
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      ...
      if (preservedPlaybackState) {
        videoElement.volume = preservedPlaybackState.volume;
      ...
      setNotice((currentNotice) => {
        ...
        return shouldApplyOpenPreferences && promptForSubtitles && !shouldForcePlayOnReady
              ...
              <div className='player-frame' style={playerFrameStyle}>
                ...
                >
                  <div className='panel-tabs'>
                    ...
                    <button
                      type='button'
                      className={panelTab === 'playlist' ? 'panel-tab active' : 'panel-tab'}
                      onClick={() => setPanelTab('playlist')}
                    >
                      {messages.panel.playlistTab}
                    </button>
                    <button
                      type='button'
                  ...
                  {panelTab === 'load' ? (
                    ...
                    <div className='panel-body playlist-panel'>
                      <div className='playlist-header'>
                        <div>
                          <p className='eyebrow'>{messages.playlist.eyebrow}</p>
                          <h3>{messages.playlist.title}</h3>
                        </div>
                        <span className='playlist-count'>
                          {playlistLoading
                            ? messages.playlist.loading
                            : messages.playlist.count(playlistVideos.length)}
                        </span>
                      </div>

                      {!videoSource.path ? (
                        <p className='helper-text'>{messages.playlist.unavailable}</p>
                      ) : playlistVideos.length > 0 ? (
                        <div className='playlist-list' role='list'>
                          {playlistVideos.map((playlistVideo, index) => {
                            const isActive = areFilePathsEqual(
                              playlistVideo.path,
                              videoSource.path,
                            );
                            ...
                            return (
                              <button
                                key={playlistVideo.path}
                                type='button'
                                className={`playlist-item ${
                                  isActive ? 'playlist-item-active' : ''
                                }`}
                                title={playlistVideo.path}
                                aria-current={isActive ? 'true' : undefined}
                                onClick={() => void playPlaylistVideo(playlistVideo)}
                              >
                                <span className='playlist-index'>
                                  {String(index + 1).padStart(2, '0')}
                                </span>
                                <span className='playlist-item-main'>
                                  <strong>{playlistVideo.fileName}</strong>
                                  {isActive ? (
                                    <small>{messages.playlist.current}</small>
                                  ) : null}
                                </span>
                                <span className='playlist-action'>
                                  {messages.playlist.play}
                                </span>
                              </button>
                            );
                          })}
                        ...
                        <p className='helper-text'>
                          {playlistLoading
                            ? messages.playlist.loading
                            : messages.playlist.empty}
                        </p>
                      )}

                      {activePlaylistIndex >= 0 ? (
                        <p className='helper-text'>
                          {messages.playlist.position(
                    ...
                    <div className='panel-body settings-container'>
                      <section className='settings-section'>
                        ...
                        <div className='settings-item'>
                          <span>{messages.behavior.endPlayback}</span>
                          <div className='settings-item-content'>
                            <select
                              className='text-input'
                              value={endPlaybackAction}
                              onChange={(event) => {
                                const nextAction = event.target.value;
                                if (isEndPlaybackAction(nextAction)) {
                                  setEndPlaybackAction(nextAction);
                                }
                              }}
                            >
                              <option value='none'>
                                {messages.behavior.endPlaybackNone}
                              </option>
                              <option value='repeat'>
                                {messages.behavior.endPlaybackRepeat}
                              </option>
                              <option value='next'>
                                {messages.behavior.endPlaybackNext}
                              </option>
                            </select>
                          </div>
                        </div>
                        ...
                        <p className='helper-text'>
                          {messages.behavior.endPlaybackHelp}
                        </p>
