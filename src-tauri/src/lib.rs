...
const HLS_SEGMENT_SECONDS: f64 = 10.0;
...
fn probe_video_duration_seconds(app: &AppHandle, path: &str) -> Result<f64, String> {
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        return Err(if stderr.is_empty() {
            "ffprobe could not read the TS duration.".into()
        } else {
            stderr
        });
    }
    ...
    let duration = raw_duration
        .parse::<f64>()
        .map_err(|_| "ffprobe did not report a usable TS duration.".to_string())?;

    if !duration.is_finite() || duration <= 0.0 {
        return Err("ffprobe reported an invalid TS duration.".into());
    }
...
fn prepare_ts_hls_stream(
    ...
    if !is_transport_stream_path(&path) {
        return Err("That file is not a TS transport stream.".into());
    }
    ...
    let audio_codec = streams
        ...
        .trim()
        .to_ascii_lowercase();
    let copy_video = video_codec == "h264";
